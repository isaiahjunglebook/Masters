...
**Built-in defense (automatic):** the app mints a YouTube *PO token* (a
BotGuard "proof of origin" attestation, generated by running Google's own
attestation VM server-side) and attaches it to every request. A missing PO
token is one of YouTube's main bot signals, so this lifts the wall in most
cases with no setup at all. It also tries five different YouTube clients per
video (web, Android, iOS, TV, embedded player) before giving up.

If downloads *still* fail with "confirm you're not a bot", the server's IP
itself is on YouTube's blocklist and no token will help. Two workarounds,
